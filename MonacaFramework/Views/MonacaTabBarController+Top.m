...
- (NSMutableDictionary *)dictionaryWithTopBarStyle {
    return [[self.ncManager.properties objectForKey:kNCPositionTop] objectForKey:kNCTypeStyle];
}
...
- (void)setLeftComponent:(NSArray *)components {
    ...
    self.leftContainers = [self createContainers:components];
...
- (void)setRightComponent:(NSArray *)components {
    ...
    self.rightContainers = [self createContainers:components];
...
- (void)setCenterComponent:(NSArray *)components {
    ...
    self.centerContainer = [[self createContainers:components] objectAtIndex:0];
...
- (MonacaTabBarController *)applyTopToolbar:(NSDictionary *)style {
    ...
    navigationBarHidden_ = hidden;
    
    // Opacity.
    NSString *opacity = [style objectForKey:kNCStyleOpacity];
    if (opacity) {
        // NOTE: Ignore the given opacity (alpha) parameter because buttons included
...
- (MonacaTabBarController *)updateTopToolbar:(NSDictionary *)style {
    ...
    // テキスト周りを変更するために追加
    NSMutableDictionary *top = [NSMutableDictionary dictionaryWithDictionary:[self.ncManager.properties objectForKey:kNCPositionTop]];
    [top setObject:topBarStyle forKey:kNCTypeStyle];
    [self.ncManager.properties setObject:top forKey:kNCPositionTop];
